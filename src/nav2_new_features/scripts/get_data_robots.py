...
class PoseSubscriber(Node):
    ...
    def process_data(self):
        if self.latest_velocity is not None and self.latest_pose is not None:
            ...
            #save velocities with np.savetxt
            
            velocities = np.array([linear_velocity_norm, angular_velocity_z])
            with open('velocities_home7.txt', 'ab') as f:
            ...
            # Append the transformation to the file
            with open('poses_home_odom7.txt', 'ab') as f:
                np.savetxt(f, flattened_transformation_odom[None, :], fmt='%.6f')

            # append timestamp in a text file
            
            with open("timestamp_7.txt", 'ab') as f:
                np.savetxt(f, [timestamp], fmt='%.6f')
            
            # Create a new directory for this experience

            # Save the arrays to text files in the new directory
